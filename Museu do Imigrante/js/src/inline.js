["load", "DOMContentLoaded"].forEach(event =>
    window.addEventListener(event, () => document.querySelectorAll("[data-graphics='inline']").forEach(makeInline))
);
...
function fetchDocument(document, callback) {
    ...
    request.addEventListener("load", () => {
        if (request.status >= 200 && request.status < 400) {
            ...
            console.warn(`An error occurred while opening "${document}"; ${request.responseURL}: ${request.statusText}`);
        }
    });

    request.addEventListener("error", () => {
        console.warn(`An error occurred while opening "${document}"`);
    });
...
function insertContent(srcElement, dstElement) {
    if (srcElement != undefined) {
        ...
        [].forEach.call(srcElement.children, element => 
            !["title", "style"].includes(element.tagName) && dstElement.appendChild(element.cloneNode(true))
        );
    }
...
function createElement(img) {
    ...
    [].forEach.call(img.attributes,
        attribute => attribute.name !== "src" && svg.setAttribute(attribute.name, attribute.value)
    );
